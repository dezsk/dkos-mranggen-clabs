      ...
      <div className="mb-6">
        ...
        <div className="bg-white shadow rounded p-4 max-w-md">
          {loading ? (
            ...
            <>
              <p>Kamar tersedia: <strong>{kamarBInfo.availableRooms}</strong></p>
              <p>Harga sewa per bulan: <strong>{formatRupiah(kamarBInfo.price)}</strong></p>
              ...
              <button
                ...
                type="button"
                disabled={kamarBInfo.availableRooms === 0}
                className={`w-full py-2 rounded font-semibold mt-3 transition 
                  ${kamarBInfo.availableRooms === 0 
                    ? 'bg-gray-400 text-white cursor-not-allowed' 
                    : 'bg-[#50A75F] text-white hover:bg-[#3c8a4b]'}`}
              >
                {kamarBInfo.availableRooms === 0 ? 'Kamar Penuh' : 'Ajukan Sewa'}
              </button>
            </>
          )}
        </div>
